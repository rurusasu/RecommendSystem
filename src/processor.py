...
class Preprocessor:
    ...
    def process(self, data: pd.DataFrame) -> dict:
        ...
        for feature_name in data.columns:
            if feature_name.startswith("interests_") and data[feature_name].dtype == "object":
                # interests_userの分割
                ...
                indices = self.lookup_layers[feature_name](interests_padded)
                processed_data[feature_name] = indices
            elif data[feature_name].dtype == "object":
                indices = self.lookup_layers[feature_name](data[feature_name].astype(str).values)
            ...
            elif pd.api.types.is_numeric_dtype(data[feature_name]):
